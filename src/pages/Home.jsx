...
const Home = () => {
  ...
  return (
    <div className='max-w-5xl p-5 md:p-10 mx-auto relative'>
        {filterValues.length>0&&<div className='flex shadow-xl justify-between items-center w-5xl py-2 md:py-auto h-auto md:h-[72px] bg-white rounded-md '>
    <div className='flex flex-wrap md:flex-nowrap'>
    {filterValues.map((item,index)=> <p key={index} className='mx-3 flex justify-center items-center my-1 md:my-auto'><span className='ml-2 px-2 rounded-l-md text-[#5CA5A5] text-[16px] font-bold bg-[#5CA5A5] bg-opacity-35 py-1'>{item}</span> <span onClick={()=>removeFilter(item)} className='cursor-pointer inline-block'>
